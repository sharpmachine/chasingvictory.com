				...
				<table class="shop_table table">
					...
					<tfoot>
						<tr class="cart-subtotal">
							...
							<td class="text-right"><?php echo $woocommerce->cart->get_cart_subtotal(); ?></td>
						</tr>
									...
									if ($_product->exists() && $values['quantity']>0) :
										echo '
											<tr class="' . esc_attr( apply_filters('woocommerce_checkout_table_item_class', 'checkout_table_item', $values, $cart_item_key ) ) . '">
												...
												'</td>
												<td class="product-total text-right">' . apply_filters( 'woocommerce_checkout_item_subtotal', $woocommerce->cart->get_product_subtotal( $_product, $values['quantity'] ), $values, $cart_item_key ) . '</td>
											</tr>';
									endif;
								endforeach;
							endif;
		...
		<div class="container">
			...
			<div class="row place-order">
				<div class="col-xs-12">
					...
					echo apply_filters('woocommerce_order_button_html', '<input type="submit" class="button alt btn btn-default btn-lg" name="woocommerce_checkout_place_order" id="place_order" value="' . $order_button_text . '" />' );
					?>
					...
					<div class="form-group">
						<div class="checkbox">
							...
							<label for="terms" class="checkbox"><?php _e( 'I have read and accept the', 'woocommerce' ); ?> <a href="<?php echo esc_url( get_permalink(woocommerce_get_page_id('terms')) ); ?>" target="_blank"><?php _e( 'store policies', 'woocommerce' ); ?></a></label>
						</div>
					</div>
					<?php endif; ?>
